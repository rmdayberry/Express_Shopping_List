...
beforeEach(() => {
  items.length = 0;
  items.push({ name: 'firewood', price: 5.00});
});

describe('GET/items',()=>{
  test('Get a lsit of items', async()=> {const res= await request(app).get('/items');
    expect(res.statusCode).toBe(200);
    expect(res.body).toEqual([{name: 'firewood', price: 5.00}]);
  });
});
...
descibe('POST /items', ()=> {
  test('Add an item to the list', async ()=> {
    ...
    .post('items')
    .send({name: 'avocado', price: 1.00});
    expect(res.statusCode).toBe(201);
    expect(res.body).toEqual({added: {name: 'avocado', price: 1.00} });
  });
...
describe('PATCH/items/:name', ()=> {
  ...
  .send({name: 'new firewood', price: 5.00});
  expect(res.statusCode).toBe(500);
  expect(res.body).toEqual({updated: { name: 'new firewood', price:5.00} });
});
